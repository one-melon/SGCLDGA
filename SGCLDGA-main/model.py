...
class SGCLDGA(nn.Module):
    def __init__(self, n_u, n_i, d, u_mul_s, v_mul_s, ut, vt, train_csr, adj_norm, l, temp, lambda_1, lambda_2, dropout,
        ...
        self.G_u_list[0] = self.E_g_0
        self.G_i_list[0] = self.E_d_0
        self.temp = temp
    ...
    def forward(self, uids, iids, pos, neg, test=False):
        if test == True:  # testing phase
            ...
            for layer in range(1, self.l + 1):
                ...
                # aggregate
                self.E_g_list[layer] = self.Z_u_list[layer]
                self.E_d_list[layer] = self.Z_i_list[layer]
            ...
            G_i_norm = self.G_i
            E_d_norm = self.E_d
            neg_score = torch.log(torch.exp(G_u_norm[uids] @ E_g_norm.T / self.temp).sum(1) + 1e-8).mean()
            neg_score += torch.log(torch.exp(G_i_norm[iids] @ E_d_norm.T / self.temp).sum(1) + 1e-8).mean()
